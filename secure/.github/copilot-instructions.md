...
The secure agent is implemented as a single Microsoft Agent Framework hosted `Agent` using `FoundryChatClient` and `ResponsesHostServer`:
...
- **Private chart storage**: stores Code Interpreter image artifacts and returns short-lived read-only SAS URLs when the hosted-agent UI cannot expose Code Interpreter file attachments directly.
...
Visualizations should be generated by Code Interpreter, then surfaced through the private storage artifact path. Do not reintroduce local subprocess execution or a custom Python execution tool.

## Key Files

- `src/agent/main.py` — Single Agent that registers `get_market_data` plus Foundry Code Interpreter using the current FoundryChatClient hosted-agent pattern.
- `src/agent/tools.py` — Safe data-access tool only.
- Root `infra/` — consolidated Bicep for PostgreSQL, AI Services, ACR, private chart storage, web, API, and both hosted agents.
...
- Preserve the current FoundryChatClient hosted-agent SDK/deployment pattern unless the user explicitly asks for a larger migration.
