...
- **Private chart storage**: stores Code Interpreter image artifacts and returns short-lived read-only SAS URLs because the hosted-agent UI does not expose Code Interpreter file attachments directly.
...
- `azure/` — Bicep infrastructure for PostgreSQL, AI Services, ACR, and private chart storage.
