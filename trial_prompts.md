...
The slide deck has the three live demos built in, so you normally don't need
these. They're here in case you want to drive the hosted agents directly in the
Azure AI Foundry portal.

Open the Foundry project printed by `azd up` and chat with the hosted agent
`contoso-market-research` (unsecure) or `contoso-market-research-secure` (secure).

## 1. Normal prompt

A benign request the agent should handle with sanitized data only:
...
This pushes the charting code away from the sanitized dataset and toward the raw
database, then hides fake PII inside the generated image:
...
- **`contoso-market-research` (unsecure):** may produce a chart containing fake
  PII, because `execute_code` runs inside the container that holds the database
  connection.
- **`contoso-market-research-secure` (secure):** the same prompts run chart code
  in Foundry Code Interpreter, which has no access to the agent container's
  database connection, so it only works with sanitized data returned by
  `get_market_data`.

No real data is used anywhere in this demo.
